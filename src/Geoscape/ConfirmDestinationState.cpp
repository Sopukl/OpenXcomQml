...
void ConfirmDestinationState::btnOkClick(Action *)
{
	...
	for (auto* craft : _crafts)
	{
		if (craft->areTooManyItemsOnboard())
		{
			...
			game.pushState(new CraftErrorState(0, message2));
			return;
		}
	}
	...
	for (auto* craft : _crafts)
	{
		if (!craft->arePilotsOnboard(game.getMod()))
		{
			game.popState();
			game.popState();
			game.pushState(new CraftNotEnoughPilotsState(craft));
			return;
		}
	}
...
void ConfirmDestinationState::btnTransferClick(Action *)
{
	...
	if (errorMessage.empty())
	{
		...
		for (auto soldierIt = currentBase->soldiers().begin(); soldierIt != currentBase->soldiers().end();)
		{
			...
			if (soldier->getCraft() == _crafts.front())
			{
				...
				targetBase->soldiers().push_back(soldier);
				soldierIt = currentBase->soldiers().erase(soldierIt);
			}
			else
			{
				++soldierIt;
		...
		targetBase->crafts().push_back(_crafts.front());
