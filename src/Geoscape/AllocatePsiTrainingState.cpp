 ...
 */
AllocatePsiTrainingState::AllocatePsiTrainingState(Base *base) : _sel(0), _base(base), _origSoldierOrder(_base->soldiers()), _doNotReset(false)
{
	// Create objects
	...
	if (game.mod()->getSoldierBonusList().empty())
	{
...
void AllocatePsiTrainingState::initList(size_t scrl)
{
	...
	for (auto* soldier : _base->soldiers())
	{
		...
		if (soldier->getCurrentStats()->psiSkill > 0 || (options1.psiStrengthEval() && game.savedGame()->isResearched(game.mod()->getPsiRequirements())))
		{
