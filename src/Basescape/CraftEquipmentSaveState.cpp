...
CraftEquipmentSaveState::CraftEquipmentSaveState(CraftEquipmentState *parent) : _parent(parent), _previousSelectedRow(-1), _selectedRow(-1)
{
	...
	{
		ItemContainer *item = game.savedGame()->getGlobalCraftLoadout(i);
		if (item->empty())
		{
		...
		{
			const std::string &itemName = game.savedGame()->getGlobalCraftLoadoutName(i);
			if (itemName.empty())
			{
...
void CraftEquipmentSaveState::saveTemplate()
{
	...
	{
		game.savedGame()->setGlobalCraftLoadoutName(_selectedRow, _edtSave->getText());
