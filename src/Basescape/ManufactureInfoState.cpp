	...
	||  action->getDetails()->button.button == SDL_BUTTON_LEFT)
	{
		...
		if (action->getDetails()->button.button == SDL_BUTTON_LEFT)
		{
			if (wasInfinite)
			{
				...
				{
					productionPossible = std::min(productionPossible, _base->getStorageItems().countOf(item.first) / item.second);
				}
...
{
	if (action->getDetails()->button.button == SDL_BUTTON_WHEELUP) moreEngineer(options1.changeValueByMouseWheel());
	else if (action->getDetails()->button.button == SDL_BUTTON_WHEELDOWN) lessEngineer(options1.changeValueByMouseWheel());
}
...
{
	if (action->getDetails()->button.button == SDL_BUTTON_WHEELUP) moreUnit(options1.changeValueByMouseWheel());
	else if (action->getDetails()->button.button == SDL_BUTTON_WHEELDOWN) lessUnit(options1.changeValueByMouseWheel());
}
