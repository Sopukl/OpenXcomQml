 ...
 */
CraftArmorState::CraftArmorState(Base *base, size_t craft) : _base(base), _craft(craft), _savedScrollPosition(0), _origSoldierOrder(_base->soldiers()), _dynGetter(NULL)
{
	// Create objects
...
void CraftArmorState::initList(size_t scrl)
{
	...
	for (const auto* soldier : _base->soldiers())
	{
		...
		{
			_lstSoldiers->addRow(3, soldier->getName(true).c_str(), soldier->getCraftString(recovery).c_str(), ltr(soldier->getArmor()->getType()).c_str());
		}
	...
	if (!(s->getCraft() && s->getCraft()->getStatus() == "STR_OUT"))
	{
		if (game.isLeftClick(action, true))
		{
			...
			else
			{
				...
				game.pushState(new SoldierArmorState(_base, _lstSoldiers->getSelectedRow(), SA_GEOSCAPE));
			}
		}
		else if (game.isRightClick(action, true))
		{
