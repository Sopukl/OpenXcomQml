	...
	_previousSort(TransferSortDirection::BY_LIST_ORDER), _currentSort(TransferSortDirection::BY_LIST_ORDER), _errorShown(false)
{
	...
	for (auto* craft : _baseFrom->crafts())
	{
		if (_debriefingState) break;
		if (craft->getStatus() != "STR_OUT" || (options1.canTransferCraftsWhileAirborne() && craft->getFuel() >= craft->getFuelLimit(_baseTo)))
		{
			TransferRow row = { TRANSFER_CRAFT, craft, craft->getName(),  (int)(25 * _distance), 1, 0, 0, -3, 0, 0, (int)(25 * _distance) };
			_items.push_back(row);
			std::string cat = getCategory(_items.size() - 1);
			if (std::find(_cats.begin(), _cats.end(), cat) == _cats.end())
			{
	...
	_cbxCategory->onKeyboardPress((ActionHandler)&TransferItemsState::btnTransferAllClick, options1.keyTransferAll());
	...
	_btnOk->onKeyboardRelease((ActionHandler)&TransferItemsState::btnQuickSearchToggle, options1.keyToggleQuickSearch());
		...
		if (transferRow.amount > 0)
		{
			...
			case TRANSFER_CRAFT:
				...
				if (craft->getStatus() == "STR_OUT")
				{
					...
					_baseTo->crafts().push_back(craft);
