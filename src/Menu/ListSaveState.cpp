...
ListSaveState::ListSaveState(OptionsOrigin origin) : ListGamesState(origin, 1, false), _previousSelectedRow(-1), _selectedRow(-1)
{
	...
	_btnSaveGame = new TextButton(game.savedGame()->isIronMan() ? 200 : 80, 16, 60, 172);
...
{
	game.savedGame()->setName(_edtSave->getText());
	...
	newFilename += ".sav";
	game.pushState(new SaveGameState(_origin, newFilename, _palette));
}
