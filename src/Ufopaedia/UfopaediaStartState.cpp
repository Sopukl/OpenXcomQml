...
namespace OpenXcom
{
	UfopaediaStartState::UfopaediaStartState() : _offset(0), _scroll(0), _maxButtons(0), _heightOffset(0), _windowOffset(0), _cats(game.getMod()->getUfopaediaCategoryList())
	{
		...
		int extraSpace = (game.getScreen()->getDY() * 2) + 20; // upper extra + lower extra + 20 pixels from the original
	...
	void UfopaediaStartState::btnSectionClick(Action *action)
	{
		for (size_t i = 0; i < _btnSections.size(); ++i)
		{
			...
			{
				game.pushState(new UfopaediaSelectState(_cats[_offset + i], _heightOffset, _windowOffset));
				break;
			}
		}
	}
