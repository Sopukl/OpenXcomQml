...
namespace OpenXcom
{
	...
	ArticleStateText::ArticleStateText(ArticleDefinitionText *defs, std::shared_ptr<ArticleCommonState> state) : ArticleState(defs->id, std::move(state))
	{
		...
		_buttonColor = game.mod()->getInterface("articleText")->getElement("button")->color;
		_titleColor = game.mod()->getInterface("articleText")->getElement("title")->color;
		_textColor1 = game.mod()->getInterface("articleText")->getElement("text")->color;
		_textColor2 = game.mod()->getInterface("articleText")->getElement("text")->color2;
		...
		// Set up objects
		game.mod()->getSurface("BACK10.SCR")->blitNShade(_bg, 0, 0);
