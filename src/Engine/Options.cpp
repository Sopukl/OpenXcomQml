...
bool init()
{
	...
	FileMap::clear(false, options1.oxceEmbeddedOnly());
	return true;
}
...
void refreshMods()
{
	...
	if (options1.oxceEmbeddedOnly() && rwops) {
...
void updateMods()
{
	...
	FileMap::setup(activeModsList, options1.oxceEmbeddedOnly());
	userSplitMasters();
...
bool isPasswordCorrect()
{
	if (_passwordCheck < 0)
	{
		std::string md5hash = md5(options1.password().toStdString());
...
void updateOptions()
{
	...
	if (CrossPlatform::folderExists(_configFolder))
	{
		if (CrossPlatform::fileExists(_configFolder + "options.cfg"))
		{
			load();
#ifndef EMBED_ASSETS
			options1.setoxceEmbeddedOnly(false);
#endif
		}
		else
		{
