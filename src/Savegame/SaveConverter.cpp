...
SavedGame *SaveConverter::loadOriginal()
{
	...
	for (size_t i = 0; i < _rules->getCountries().size(); ++i)
	{
		...
		country->funding().clear();
		_save->countries().push_back(country);
	}
...
void SaveConverter::loadDatIGlob()
{
	...
	for (size_t i = 0; i < _rules->getCountries().size(); ++i)
	{
		Country *country = _save->countries().at(i);
		...
		graphVector(country->funding(), month, _year != _mod->getStartingTime().getYear());
	}
	for (size_t i = 0; i < _rules->getRegions().size(); ++i)
	{
...
void SaveConverter::loadDatXcom()
{
	...
	for (size_t i = 0; i < ENTRIES * MONTHS; ++i)
	{
		...
		{
			_save->countries().at(j)->getActivityXcom().push_back(score);
		}
		// region
		else
		{
...
void SaveConverter::loadDatAlien()
{
	...
	for (size_t i = 0; i < ENTRIES * MONTHS; ++i)
	{
		...
		if (j < _rules->getCountries().size())
		{
			_save->countries().at(j)->getActivityAlien().push_back(score);
		}
		// region
		else
		{
...
void SaveConverter::loadDatDiplom()
{
	...
	for (size_t i = 0; i < _rules->getCountries().size(); ++i)
	{
		...
		for (size_t j = 0; j < MONTHS; ++j)
		{
			...
			funding *= 1000;
			income[j] += funding;
			country->funding().push_back(funding);
		}
...
void SaveConverter::loadDatProject()
{
	...
	for (size_t i = 0; i < _save->bases().size(); ++i)
	{
		Base *base = _save->bases().at(i);
